...
1. **状態探索OS 第2検証=右裾残差の軸構造と丘性 (次にやること)**: cfs221でvola除去後も左裾独立の右裾残差(0.22-0.35)が残った=壁を抜ける唯一の候補。この残差がどの軸(price/gap/volume/momentum)の組み合わせから生まれ、丘として存在するかを検証。残差を悲観で切らず候補として正体を見極める。これがPROBE Q2の最優先
2. **地質調査は完了 (cfs218-221、両者同意・CFS_MAP記録済)**: 状態空間は地形(ノイズでない、forwardシャッフルで消失)/地形はvola主導(cfs207の壁への警戒材料)/vola除去後も左裾独立の右裾残差あり(唯一の候補)/構造は小ノイズに耐える(針でない)
3. **判定で分岐**: 残差が新しい軸構造で丘なら丘探索へ進む / 残差もvola相関の焼き直し・針なら状態探索OSは手持ちで限界(前提変更の議論へ、10x不変)
