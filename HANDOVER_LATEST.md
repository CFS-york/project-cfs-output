...
- SL(緩い-8〜-20%)は真multを全て下げる(逆効果)。mp並列は複利破壊。既存の改善手段は全て無効と判明
- 飛躍軸探索: τ軸3段棄却、investor軸保留、H4e系打ち止め(崩壊理論)

### 直近の最重要発見 (2026-06-02)
**現在地は 1.141x (真mult, mp=1) に最終確定。10xまで8.76倍不足。既存手段では届かない**。
- 既存multは全て過大: 外挿式(netをTAX按分で甘く+誤差増幅) + cfs11 event駆動simのバグ
- cfs13で複数実装を突き合わせ確定畳み(全額逐次)=mp=1の正と判明 → 真mult 1.141x
- SL(-8〜-20%)全て逆効果(mult低下)、mp並列は複利破壊。既存cellの小改良では10x不可
- H4e系打ち止め(崩壊理論)、τ/investor軸も尽きた
- ★次の本質課題: 「1トレードあたりnet期待値を構造的に上げる別エッジ」。小改良でなく新エッジが必須
...
- **(2026-06-02) 緩いSL(-8〜-20%)はこのcellで全て真mult低下(逆効果)。mp並列は複利破壊。集中逐次mp=1が最良**
...
### 優先 1: 1トレードあたりnet期待値を構造的に上げる新エッジ
- 現在地1.141x(8.76倍不足)。既存cell小改良(SL/mp/パラメータ)では届かないと検証で確定
- 必要なのは「1トレード単位のnet期待値そのもの」を上げる別エッジ。複利は経路依存だが土台は1トレードEV
- 評価は正式複利sim(確定畳み=全額逐次mp=1)で統一。外挿mult・cfs11 event駆動simは使わない(誤り確定)
- 探索済で尽きた軸: 既存gap×ext×universe×HIGH20(1.141x上限)、τ軸、investor軸、H4e。これら以外の新次元が要る
- 未着手の在庫: sector_master, listed_info, market_segments, h4e_features(特徴量側), 信用残, 業種ローテ 等
...
- accept内の時間的重なり0件(mp=1トレードは分離) → ARK仮説(重なりでalloc分割)は外れ
- 真因: cfs11のevent駆動simがidx管理バグ(cfs13再現でmult=1.000,MDD=0%と明らかに破綻)
- 確定畳み(全額逐次=mp=1の正しい計算)=1.141x。cfs12とも一致 → ★真mult 1.141x 確定
...
- 2026-06-02 v1.5 cfs12-13反映 (ARK全文更新) ★★現在地 最終確定
  - cfs12 緩いSLは全て逆効果(真mult低下)
  - ★cfs13: cfs11 event駆動simがバグと判明。確定畳み(全額逐次)=mp=1の正で真mult=1.141x確定
  - 数字変遷 2.887→4.311→2.062→1.141 に終止符。真の現在地1.141x、10xまで8.76倍不足
  - 優先1を「1トレードnet期待値を構造的に上げる新エッジ」に更新(小改良では届かないと確定)
