...
### ★最重要: 「2つ目のnetプラスedge」を別の執行骨格で探す (制約据え置き, ヨーク方針確定済=探索のみ)
- 今日14本で判明: netプラスedgeは1つ(gap急騰top1)だけ。betting/N増/exit/mpはそれを束ねる技術で、2つ目edgeが無いと無力
- ★1つ目edgeの変種(翌日寄付×順張りモメンタム)はgap/τ/H4e/平均回帰/ファンダ 全敗。同じ骨格では2つ目は出ない
- ★未踏の執行骨格(まだ一度も検証していない):
  (A)当日引けexit = 翌日寄付entry→当日引けで決済(オーバーナイトリスク排除の日中モメンタム)。回転速くN増も
  (B)寄り付き前の気配/板情報 を使うシグナル(J-Quants範囲確認要)
  (C)複数銘柄の相対強弱 = 業種内/市場内で相対的に強い銘柄だけロング(sector_master活用)
  (D)出来高急増を主軸(gap従)= gap無しでvol_ratio単独の異常出来高シグナル
- 着手法: 別骨格を1つ選び、まず1トレードnet期待値を確認(cfs7/15/16と同手順)→netプラスなら戦略化・複利sim
  ★2つ目のnetプラスedgeが出れば、betting/N増で1つ目と束ねて複利を伸ばせる(その時こそ分散が効く)
...
**■ ★★今日(cfs6-19, 14本)の核心結論**
- 全ての道が「netプラスedgeは4000-7000 gap急騰top1の1つだけ」に収束
- entry別軸/exit/mp/N増/betting/別次元(ファンダ)/逆張り 全敗。原因は全て「2つ目のnetプラスedgeが無い」
- 1.141xはその1つのedgeの複利上限。betting/N増は束ねる技術で2つ目edgeが無いと無力
- ★必要なのは2つ目のnetプラスedge。それは翌日寄付×順張りモメンタム変種では出ない(全敗)→別の執行骨格へ
