...
- ★次の方向2案: (A)既存1.141x戦略を時間的に重ならない複数系統で逐次回しN増(複利でmult向上, 10x逆算のN増路線)
  (B)単一銘柄選択でなく複数銘柄の相対関係(業種内relative strength/ペア)で別の収益構造
  - 10x逆算: net 0.23〜0.68%/trade × N 340〜1020(N500超コスト割れ)
...
### 優先 1: 既存1.141xモメンタムのトレード機会を逐次で増やしN増(複利でmult向上)
- ★最有望。10x逆算(net0.285%維持でN111→340〜1020に増やせば複利でmult大幅向上)に直接効く
- mp並列は複利破壊と確定(cfs11)。だが「時間的に重ならない複数系統を逐次合算」ならNを増やせる
- 既存最良(gap0.065×HIGH20×4000-7000)に加え、別universe/別gap閾値の独立モメンタム系統を複数用意し、
  時間軸で重複しないトレードを合算 → 確定畳み複利sim(mp=1の単一資金を複数系統で逐次運用)
- phase_v3_portfolio_2sysが外挿でやろうとした事を、正式複利simで正しく検証
- 着手法: 複数モメンタム系統のトレード列を生成→時間順マージ(重複は先着優先)→確定畳みsimで真mult

### 優先 2: 複数銘柄の相対関係(業種内relative strength/ペア)
- 単一銘柄選択でなく相対構造。価格時系列・ファンダの単独軸が全滅した別の収益構造
- sector_master(業種)で業種内相対強弱、または同業ペアの乖離回帰 等

### 参考: 発表後ドリフトは全次元で取れない(確定)
- 価格/ファンダ/H4e/τ/平均回帰 全てで「翌日寄付エントリーのドリフト取り」はnet edgeにならない
...
- ★構造確定: 「シグナル/発表後に翌日寄付で素直に入る」は価格・ファンダ・H4e・τ・平均回帰 全次元でedge無し
  例外はgap急騰モメンタム短期(1.141x)のみ。次はトレード機会を逐次でN増 or 相対関係へ
...
- 2026-06-02 v1.8 cfs16反映 (ARK全文更新) ★ファンダ次元も不成立・構造的洞察
  - cfs16 ファンダ変化率(利益加速・予想修正)も素のnet edge無し
  - ★構造的洞察: 「シグナル後に翌日寄付で素直に入る」は全次元でedge無し。例外gap急騰モメンタム短期のみ
  - 優先1を「既存1.141xモメンタムのトレード機会を逐次でN増(複利でmult向上)」に更新。優先2=相対関係
