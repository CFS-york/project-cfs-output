...
## 3. 次アクション (優先順、 2026-06-19 [LOT-H1]却下・[LOT-I1] price×momentum発見後)

[LOT-H1]却下(price+gap地形は総当たり回避で閉じ、丘Aは非接続)。[LOT-I1]でprice×momentumの本物の丘を発見、身分監査(B1-枝2)通過。次アクション:

1. **[LOT-I1] B1-枝3: price×momentum丘の資金管理・10x距離 (次にやること)**: B1-枝1(cfs227)で針でない丘を確証、B1-枝2(cfs228)で身分監査通過(price帯10-30%・mom窓10/40・前後半すべてで丘が残る=条件点でなく本物の丘、勝ち分散top5%162%/win率52%)。だが各期間単体ではmult≈1.07と弱く、全期間1.4xも10x要求(hold40でr≥32.5%)に遠い。次はhold/K・帯最適化・退出設計で平均rが10x方向に伸びるか。伸びれば採用候補、頭打ちならprice×momも丘だが10x非接続=状態探索OSの含意へ。これがPROBE Q2の最優先=「[LOT-I1] price+momentum軸は資金曲線で平均rを伸ばせるか」
2. **[LOT-I1]の確定事項 (cfs226-228、GPT監査済)**: price下位×momentum上位[70-90]=本物の丘(針でない、price軸/mom窓/期間で頑健、勝ち分散)。構造=「緩やか上昇(+2〜10%/20日)に乗り始めた低位株」。price+gap(mult1.055x/top5%793%/MDD14%)より良い(mult1.4x/top5%162%/MDD8%)。ただし各期間1.07と弱く10xに遠い(現状r+4.2% vs 要求32.5%)
3. **判定で分岐**: B1-枝3でhold/K・退出が平均rを10x方向に伸ばす→採用候補edge / 伸ばせない→price×momも丘だが10x非接続。状態探索OSで3丘(price+gap/price+vola/price+mom)とも10x非接続なら手持ち日足の限界が濃厚(前提変更議論、10x不変)
4. **未監査(B1-枝3以降)**: leave-year-out(C期間は前後半のみ)、momentum窓の細分化、なぜmomentum上位が効くかの構造的理解
5. **[LOT-H1]却下の記録 (誤読防止)**: price+gap地形全体を否定したのではない。丘Aは非接続だが、地形全体を閉じるには総当たりが必要でOS理念に反するため、テーマとして却下した
6. **状態探索OSの設計原則(5回の差し戻しで確定)**: 高さでなく体積/探索後評価を避ける/個別予測(p0)でなく分布の連続性/全状態を同じ軸で殺す(P1救済しない)

旧次アクション([LOT-H1])は完了(cfs223-225でprice+gap丘A非接続、却下)。分足は cache に無し(取得はヨーク戦略判断)。
