...
### 直近の最重要発見 (2026-06-02)
**1.141xはgap急騰モメンタムシグナルの証明された天井。entry/exit/資金 全て最適化済**。
- 真mult 1.141x(mp=1, net0.285%/trade × N111)。確定畳みsim cfs12/13/14が一致。10xまで8.76倍不足
- exit軸決着: SL(cfs12下方向)もtrailing(cfs14上方向)も全て悪化。固定短期exitが既にこのシグナルの最適解
  - 理由: gap急騰高値更新銘柄はentry直後がピークで素直に下げる→短く取るが正解、動的exitで下落を食らう
- entry/exit/mp 全軸を解剖し1.141xが箱の上限と確定。箱の中の最適化は完了
- ★次の本質課題: gap急騰top1とは別の、より高頻度でnet期待値を保てる新シグナル源
  - 10x逆算条件: net 0.23〜0.68%/trade × N 340〜1020(=gross 0.83〜1.40%/trade)。N500超はコスト割れ
...
- **(2026-06-02) ★1.141xはgap急騰モメンタムシグナルの証明された天井(entry/exit/mp全軸最適化済)。箱の中は完了**
...
### 優先 1: gap急騰top1とは別の新シグナル源 (より高頻度・net期待値保持)
- 1.141xはgap急騰モメンタムの天井(entry/exit/mp全最適化済)と確定。このシグナル系統の改良では10x不可
- 必要なのは別のシグナル源。10x逆算条件: net 0.23〜0.68%/trade × N 340〜1020(N500超はコスト割れ)
  - 現状gap系=net0.285%×N111。net維持でNを3〜9倍に増やすか、netを2倍以上にする新シグナルが要る
- gap急騰=モメンタム順張り。未検証の別系統: 平均回帰(押し目買い)、ペア/裁定、低ボラ高頻度、
  セクター内相対強弱、新規上場/イベントドリブン 等。モメンタムとは別の収益構造を持つ系統
- 評価は確定畳み複利sim(全額逐次mp=1)で統一。外挿mult・event駆動simは使わない(誤り確定)
- 着手法: 新シグナル源を1つ選び、まず1トレードnet期待値とN(頻度)を測る→10x逆算条件と照合
...
- **(2026-06-02) gap急騰モメンタムシグナルの改良全般。entry/exit/mp全軸で1.141x天井と証明。別シグナル源へ**
...
- 既存cellシグナル+trailing stop(trail5/8/10/15%×maxhold10/20/40)。確定畳みsim、fixed対照=1.141x再現(整合OK)
- trailing全12セル0.46x以下、net_mean全てマイナス(-0.99〜-2.96%)、wr低下
- gap急騰高値更新銘柄はentry直後ピークで素直に下げる→trailで押し目に叩き出され下落を食う
- 結論: exit軸(SL下方向cfs12/trailing上方向cfs14)両方向で1.141x超えられず。固定短期exitが最適
- ★entry/exit/mp全軸を解剖し1.141xはgap急騰モメンタムの証明された天井と確定
...
- 2026-06-02 v1.6 cfs14反映 (ARK全文更新) ★exit軸決着
  - cfs14 trailing exitも全て悪化。SL(下)もtrailing(上)もexit軸は固定短期が最適と確定
  - 1.141xはgap急騰モメンタムの証明された天井(entry/exit/mp全軸最適化済)
  - 優先1を「gap急騰top1とは別の新シグナル源(高頻度・net期待値保持、10x逆算条件と照合)」に更新
