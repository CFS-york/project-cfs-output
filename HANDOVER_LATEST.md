...
## 3. 次アクション (優先順、 2026-06-18 P1棄却・状態探索OS転換後)

P1を過適合確定で棄却(判定D=GUILTY)。検証方針を因子探索→状態探索OSへ転換。次アクションはこの転換に沿う:

1. **[LOT-H1] price+gap右裾地形を資金曲線で回し10x接続するか (次にやること)**: [LOT-G1]完遂(cfs218-222、探索価値ある右裾地形あり=price+gapに左裾独立の頑健な丘)。次はこの地形を K/hold/退出/16m換算で資金曲線上に回し、平均rが10x方程式(hold20でr≥15.13%)に接続するか検証。ここで初めて採用候補edgeか決まる。これがPROBE Q2の最優先
2. **[LOT-G1]完遂済 (cfs218-222、両者同意・CFS_MAP記録)**: 状態空間は地形(ノイズでない)/地形はvola主導(警戒材料)/vola除去後もprice+gapに左裾独立(同測定で鏡像でない)の右裾地形が残りσ摂動に耐える(針でない丘)。ただし地質調査の完了であって資金曲線edge確定でない(地質≠資金曲線)
3. **判定で分岐**: [LOT-H1]でprice+gap地形が資金曲線で平均rを改善し10x接続→採用候補edge / 接続しない(低位の大化けは平均rが上がらない既知の壁の再来)→状態探索OSは手持ち限界(前提変更議論、10x不変)
3. **状態探索OSの設計原則(5回の差し戻しで確定)**: 高さでなく体積/探索後評価を避ける/個別予測(p0)でなく分布の連続性/全状態を同じ軸で殺す(P1救済しない)
4. **回収項目(監査一段落後)**: P1_DEFINITION完全仕様化、ARK共通規律のファイル化

旧次アクション(判定D最終確定)は完了(cfs214-217でP1過適合確定・棄却)。分足は cache に無し(取得はヨーク戦略判断)。
