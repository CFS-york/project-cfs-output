...
### ★★初の有望edge発見 (2026-06-03, cfs23)
**past_big(過去60日の大化け回数)が+30%大化けの最強予測。Q5で到達率17.99%=基準6倍(lift6.08)**。
...
- ★解釈: 「大化けする銘柄は繰り返し大化けする」+「高ボラ・低位」。cfs8の高変動説を定量再確認
- 次: past_big × vola20 × 低価格 の組合せでprecision最大化。precision30-40%なら11回当てるのに30-40回撃てば10x射程
- 注意: past_big Q5でも82%は外れ。10xには外れ時の損失制御が不可欠(cfs9のTP失敗=外れ下落に食われた)
...
- **(2026-06-03) ★★past_big(過去60日大化け回数)が+30%大化けの最強予測。Q5到達率17.99%=lift6.08。大化け体質は持続(cfs23)**
...
- ★cfs23で有望特徴発見: past_big(lift6.08) > vola20(2.57) > range20(2.47) > price低位(1.88)
- 次の一手: past_big × vola20 × 低価格 の組合せprecision最大化(cfs24)。curve fit警戒で2-3特徴まで、各セルn確保
- precision向上後: 撃ち方(entry/exit/損切り)を設計し外れ82%の損失制御。確定畳み複利simで真mult測定
...
**■ ★★cfs23 +30%大化けの単一特徴リフト探索 (初の有望edge)**
- クリーン母集団, 基準+30%到達率2.96%。単一特徴Q5リフト:
  ★past_big(過去60日大化け回数) 17.99% lift6.08(単調) / vola20 7.61% lift2.57 / range20 7.32% lift2.47
  / price低位Q1 5.57% lift1.88 / vol_ratio・vol_abs 弱い(~1.2)
- ★大化け体質は持続する(past_bigが圧倒)。高ボラ・低位も効く。cfs8高変動説を定量確認
- 次: past_big×vola20×低価格でprecision最大化。但しpast_big Q5でも82%外れ→損失制御必須
...
- 2026-06-03 v2.2 cfs21-23反映 (ARK全文更新) ★★初の有望edge発見
  - cfs21/22 価格データ異常検出→拡張BLACKLIST33銘柄確定(clean_blacklist.csv, 自動適用)
  - ★cfs23 +30%大化けの最強予測=past_big(過去60日大化け回数)Q5到達率17.99%=lift6.08。vola20/range20も有効
  - 「大化けする銘柄は繰り返す」。次はpast_big×vola20×低価格でprecision最大化(cfs24)
