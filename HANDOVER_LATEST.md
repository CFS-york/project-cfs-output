...
- **(2026-06-02) ★★核心: netプラスedgeは4000-7000 gap急騰top1の1つだけ。betting/N増/exit/mpは束ねる2つ目edgeが無いので無力。必要なのは2つ目のnetプラスedge(別の執行骨格で)**
...
- 着手前の必須: 価格データ外れ値の除去(cfs20完全予知でmaxup9億%=異常。分割調整ミス等。BLACKLIST14では不足)
  - 大化け識別は外れ値に汚染されやすい。まず異常銘柄日を検出・除外する土台整備から
...
- entry別軸/exit/mp/N増/betting/ファンダ/逆張り 全敗。但しこれは「1.141xの的」での話
- 発想転換後: 1.141xは探索の中心から外し、+30%大化け識別という別の的を狙う
