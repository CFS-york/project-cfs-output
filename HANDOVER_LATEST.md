...
- exit軸(SL下/trailing上)・mp並列・entry側全軸・betting・N増、全て尽きた。1.141xは現時点の上限
...
**10xの的(cfs20)**: +30%を11回で10x。+30%大化けは1日117件実在(全銘柄日3.17%)。
...
- cfs26: gap<=-7%×寄り→3日後寄り = net+0.098% wr54%(横断平均)
- cfs27深掘り: 最良 gap<=-7%×hold10×高出来高 net+0.267% wr54.6% n7524。+0.5%超セルなし
- ★★cfs28 複利sim判定: mult0.035x/net-3.27%/MDD96.5%で壊滅。「横断平均netプラス」≠「複利実行でプラス」
  - hold長い少回数edgeは逐次で拾うN(7522→95)が偏り序盤大負けで複利死
- ★2edge束ね(順張り+逆張り)は無効。月次相関0.112だがedge2が複利で壊滅する以上束ね意味なし
...
- ★cron auto_handover 復旧+自己検知ループ稼働(2026-06-03): handover_runnerをJSON→マーカー方式に、max_tokens16000、cron_status.json記録
...
- ★★2edge束ね(順張り+逆張り)無効。月次相関0.112だがedge2が複利で壊滅し束ね意味なし
...
- 案: HANDOVER_LATEST=cron圧縮active版(16KB,起動時必読) + HANDOVER_FULL=全履歴版
...
- 合算も全パターン0.07〜0.18xで崩壊。月次相関0.112だがedge2が複利で死ぬので束ね無意味
- sim実装3度目の同型ミス(entry_t基準でN129)をassertが検出→sig_t基準(cfs19規則)に修正
...
- オーバーナイト(引け→翌寄り)はnet-0.53% wr24%で否定
...
- 2026-06-03 v2.6 cfs27/28反映: ★★逆張りは複利で壊滅(横断平均+0.267%→複利0.035x)・2edge束ね無効・教訓確定・cron復旧・次はcfs29業種内相対強弱