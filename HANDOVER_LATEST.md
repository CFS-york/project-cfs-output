...
### 探索 状況
- ★**2026-06-02 重大修正: 既存の「mult 2.887x/4.311x」は (1+R)^N16 外挿で過大評価だった**
- 正式複利sim(現金残高ベース逐次複利・slot制約)で既存最良cellの**真mult = 2.062x(mp=1)**を確定
  - cell: gap=0.065、ext=4、universe=4000-7000、HIGH20、p1u=2%、p1d=-6%、Ch=7
  - **真の達成率 20.6%、10xまで約4.85倍不足**(従来認識2.887xより現実は遠かった)
  - mp=2以上は複利破壊(mp2=0.031x,mp3以上ほぼ全損MDD99%)。集中逐次mp=1が複利最良
  - mp=1でもMDD 33.3%(高ボラ無SLの代償)
- 飛躍軸探索: τ軸3段棄却、investor軸保留、H4e系打ち止め(元々崩壊理論、深掘りは寄り道だった)

### 直近の最重要発見 (2026-06-02)
**既存の現在地(mult)は外挿式で過大評価。真の現在地は 2.062x(正式複利sim, mp=1)**。
- 既存全scriptのmultは R=ev*(1-TAX*wr)-COST; mult=(1+R)^(n/5*340/252) の外挿
  → netを過大評価(TAXをwr按分で甘く)、n=111平均を~30乗で誤差増幅
- 正式複利sim(alloc=capital/空きslot,逐次,slot制約,最小ケースverify済)の真mult=2.062x(mp=1)
- ★mp(並列)は複利破壊: mp2=0.031x, mp3以上ほぼ全損。集中逐次が複利最良。「N増でmult増」は逆
- H4e系(cfs7-10)は打ち止め: 元々崩壊理論、既存gap戦略と水と油(D5除外でn111→37,mult4.31→0.97)
...
- **(2026-06-02) ★既存mult(2.887x/4.311x)は外挿式で過大。正式複利simの真mult=2.062x(mp=1)。10xまで4.85倍不足**
- **(2026-06-02) mp(slot並列)は複利破壊(mp2=0.031x,mp3+全損)。集中逐次mp=1が複利最良。MDD33%**
- **(2026-06-02) H4eは崩壊理論で10x不可(既存gap戦略と水と油、新規大化けもEVマイナス)。cfs7のdip予測力は事実だが使い道なし**

---

## 3. 次アクション (優先順)

### 優先 1: 既存最良cellの複利効率最適化 (真mult 2.062x を超える)
- 真の現在地=2.062x(mp=1)。これを正式複利simで超える方向を探す
- 次検証(cfs12予定): 既存最良cellに緩いSL(-10〜-20%)を足し、MDD33%を下げて真mult(正式sim)が上がるか
  - cfs9でSLは大化け狙いには効かなかったが、複利のドローダウン抑制では別の効き方の可能性
- 評価軸を「外挿mult」から「正式複利sim mult」に全面切替(外挿は誤差増幅で信頼できないと確定)
- mp=1集中逐次を基本とする(並列は複利破壊と確定)
...
- **(2026-06-02) ★ (1+R)^N16 外挿mult式。netを過大評価+誤差増幅。今後は正式複利simのみで評価**
...
**■ ★cfs11 正式複利sim (現在地の真値確定)**
- 既存全multは(1+R)^N16外挿と判明(portfolio script読解で確定)。現金残高ベース逐次複利を一度も通してない
- 正式複利sim(alloc=capital/空きslot,entry:capital-=invest,exit:capital+=invest*(1+net),最小ケースverify済)
- ★真mult: mp=1 **2.062x**(MDD33.3%,accepted111) / mp2 0.031x / mp3 0.001x / mp5 0.001x
- mp2以上は複利破壊(全額少数銘柄集中投下→負けが資金直撃,MDD97-99%)。集中逐次mp=1が複利最良
- 旧外挿4.311x/2.887xは幻。真の現在地2.062x、達成率20.6%、10xまで4.85倍不足
...
- 2026-06-02 v1.4 cfs9-11反映 (ARK全文更新) ★現在地の根幹修正
  ...
  - ★cfs11: 既存mult(2.887x/4.311x)は外挿で過大。正式複利sim真mult=2.062x(mp=1)が真の現在地
  - mp並列は複利破壊と確定。評価軸を外挿→正式複利simに全面切替
  - 次アクション優先1を「既存最良cellの複利効率最適化(緩いSLでMDD抑制→真mult向上)」に更新
