...
# Construction順序 再修正（2026-06-26、STSD挿入。Freeze Principle運用証拠による2回目の改訂）

★Teacher Construction（god_eye_min/v2/v3）を実装した運用の中で判明: 教師を先に作ると、神の目が「最終資産max」でただ最大化し、2112倍〜数千倍の非現実的な経路になる。原因は**Goal(10x)成立条件を持たずに最大化したから**。Teacher Construction自体が研究目的化し、Goalから視点が外れていた（GPT通達 + ヨーク同意）。

## 再修正後のConstruction順序（STSD挿入）
```
Goal
 → Success Trade Simulation Design（STSD: Goal成立条件・理論値・期待値を「領域」として設計）
 → Teacher Construction（実データから、STSDの条件領域内で成功シミュレーションを抽出）
 → Teacher Analysis（共通構造を抽出）
 → Theory → Logic → Signal → 実データ検証 → CFS手法
```

## Success Trade Simulation Design（STSD）とは
**16ヶ月10xを成立させるために必要な条件を、Goalから逆算して「領域」として定義した設計書**（一点でなく、成立し得る条件空間）。GPT承認済み。

Goal成立条件 = 以下4条件群の積集合 A∧B∧C∧D:
- **A 収益構造**: 平均リターン/トレード数・回転率/勝率とRR（10xを複利で生む、連動する領域）
- **B リスク構造**: 許容DD/分散（現実に耐えられる、DD95%経由は不成立）
- **C 実装構造**: 流動性/コスト/Gap約定（現実に約定できる）
- **D 普遍構造**: 異常相場非依存/観測可能性（再現可能、D2観測可能性が無いと神の目でしか成立せず実運用不能。D2の中身はTeacher Analysisで探す）

★教師は、この条件領域内で実データから抽出される実例。領域を先に定めるから、領域外（数千倍）の経路を抽出しない（**仮説、検証はTeacher再構成+Analysis後**）。
★数値（閾値）は概念設計では決めない。実データ検証で決める。

## 重要な位置づけ（OS規律、GPT監査の4 Critical点）
- Teacher・God-Eyeは研究目的でなく、Goal達成の**手段**（教材）。Teacherを最適化することが研究になってはいけない。
- 今後のCritical OS違反: ①Goalから外れる ②STSDより先にTeacherを作る ③Teacherを目的化する ④実データという前提を壊す。
